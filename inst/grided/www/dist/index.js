...
window.onload = function () {
  ...
  var styles_for_container = find_rules_by_selector_1.find_rules_by_selector("#grid_page", "gridTemplateColumns"); // Setup some basic styles for the container to make sure it fits into the
  // grided interface properly. 

  styles_for_container.height = "100%";
  styles_for_container.width = "100%"; // Only set a default gap sizing if it isn't already provided
  ...
  if (app_mode === "ShinyExisting") {
    ...
    var current_gap = get_gap_size(styles_for_container); // If grided is running on an existing app, we need to parse the children and
  ...
  function update_grid(opts) {
    ...
    var old_gap = get_gap_size(styles_for_container);
  ...
  function gen_code_for_layout() {
    ...
    var css_code = misc_helpers_1.concat_nl.apply(void 0, __spreadArray([container_selector + " {", "  display: grid;", "  grid-template-columns: " + styles_for_container.gridTemplateColumns + ";", "  grid-template-rows: " + styles_for_container.gridTemplateRows + ";", "  gap: " + get_gap_size(styles_for_container), "}"], element_defs));
